...
//									: null;
	...
	public function css($attr, $val = '') {
		...
		{
			list($style_name, $style_value) = explode(':', $style, 2);
			$style_name = ltrim(rtrim(trim($style_name)));
			$style_value = ltrim(rtrim(trim($style_value)));
			if($style_name == $attr)
			{
				if($val)
				{
			...
			if($val)
			{
				$newStyles[] = $style_name.':'.$style_value;
			}
		}

		if($val)
		{
			$this->attr('style', implode(';', $newStyles));
			return $this;
		}
		return '';
	}
